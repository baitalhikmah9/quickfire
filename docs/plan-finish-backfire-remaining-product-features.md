# Plan: Finish Backfire Remaining Product Features

## Summary
Finish the unchecked scope in `docs/backfire-full-product-plan.md` in TDD-first slices. The remaining work breaks into backend integrity, auth/device/offline sync, advanced gameplay, economy/store/admin, profile/history, and release hardening.
